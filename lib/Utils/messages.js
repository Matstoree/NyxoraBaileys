...
const generateWAMessageContent = async (message, options) => {
    ...
    const ctx = message.contextInfo

    if (ctx.externalAdReply?.thumbnailUrl && !ctx.externalAdReply.thumbnail) {
        try {
            ctx.externalAdReply.thumbnail = await axios_1.default
                .get(ctx.externalAdReply.thumbnailUrl, { responseType: 'arraybuffer' })
                .then(r => r.data)
        } catch {}
    }

    m[messageType].contextInfo = ctx
}
